...
if(isset($_POST['signup']) && $conn){
   ...
   /* CHECK EXISTING USER */
   if(empty($message)){
      $check = mysqli_query($conn, "SELECT * FROM students WHERE email='$email' OR student_usn='$usn'");
      ...
      if(mysqli_num_rows($check) > 0){
         ...
         $insert = mysqli_query($conn, "INSERT INTO students (student_name,student_usn,email,phone,password) VALUES ('$student_name','$usn','$email','$phone','$hashed_password')");
...
if(isset($_POST['login']) && $conn){
   ...
   $query = mysqli_query($conn, "SELECT * FROM students WHERE student_usn='$usn'");
   ...
   if(mysqli_num_rows($query) > 0){
      ...
      if(password_verify($password, $student['password'])){
         ...
         $_SESSION['student_usn'] = $student['student_usn'];
